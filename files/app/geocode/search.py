...
Search strategy (Photon API v1 — komoot/photon docs):
  - Accuracy from location bias (lat/lon + zoom + location_bias_scale), not raw limit=25.
  - Tier 1: zoom=16, scale=0.12 — local autocomplete
  - Tier 2: zoom=13, scale=0.35 — broader when tier 1 sparse
  - Upstream limit 12/call; merge → up to 25 for UI.
"""
...
LONDON_BBOX = (-0.25, 51.46, 0.05, 51.56)

PHOTON_UPSTREAM_LIMIT = 12
DISPLAY_CAP_DEFAULT = 25
...
async def _get_client() -> httpx.AsyncClient:
    ...
    if _http_client is None or _http_client.is_closed:
        _http_client = httpx.AsyncClient(timeout=4, headers=HEADERS)
    return _http_client
...
def _brand_token(raw: str) -> str | None:
    ...
    if len(cleaned) < 2:
        return None
    tokens = [t.replace("'", "") for t in cleaned.split() if t and t not in _BRAND_STOP]
    if not tokens:
        return None
...
async def _photon_search(
    ...
    *,
    near_lat: float | None = None,
    near_lon: float | None = None,
    limit: int = PHOTON_UPSTREAM_LIMIT,
    use_bbox: bool = True,
    zoom: int = 16,
    location_bias_scale: float = 0.12,
) -> list[dict]:
    """Photon with official location-bias params (komoot/photon api-v1)."""
    ...
    params: dict = {
        "q": query,
        "limit": min(limit, PHOTON_UPSTREAM_LIMIT),
        "lat": bias_lat,
        "lon": bias_lon,
        "zoom": zoom,
...
async def _photon_tiered(
    ...
    *,
    ...
    return _sort_places(_dedupe_places(local + broad))[:cap]
...
async def _free_fast_search(
    ...
    *,
    ...
    if len(photon) >= min(3, cap):
        provider = "photon+tier" if len(photon) >= 5 else "photon"
        if token:
            provider = "photon+brand"
        return _sort_places(photon)[:cap], provider

    nominatim_task = asyncio.create_task(
        _nominatim_search(search_q, limit=8, near_lat=sort_lat, near_lon=sort_lon)
    )
    try:
    ...
    provider = "photon+nominatim" if extra else "photon+tier"
